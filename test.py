...
def launch_gui():
    global fitted_eta, fitted_beta, selected_model, fitted_models, loaded_data
    root = tk.Tk()
    ...
    ttk.Checkbutton(root, text="使用擬合 η 取代推導值", variable=use_eta_var).grid(row=9, column=0, columnspan=2)

    model_var = tk.StringVar(value="Weibull")
    ttk.Label(root, text="選擇模型:").grid(row=9, column=2)
    model_menu = ttk.Combobox(root, textvariable=model_var, values=["Weibull", "Lognormal", "Exponential"])
    model_menu.grid(row=9, column=3)
    ...
    def calculate():
        try:
            ...
            mode = mode_var.get()
            eta = fitted_eta if use_eta_var.get() else None
            sigma = None
            if model == 'Lognormal' and model in fitted_models:
                sigma = fitted_models[model][0][0]
            if model == 'Weibull' and not eta:
                eta = compute_eta(Td, R, beta)
    ...
    def load_csv():
        ...
        try:
            ...
            model_var.set(best_model)
            model_menu["values"] = list(fitted_models.keys())
            messagebox.showinfo("AI Decision", f"最佳模型為：{best_model}\n\n{summary}")
            plot_survival_comparison(data)
        except Exception as e:
            messagebox.showerror("CSV Error", str(e))

    ttk.Button(root, text="Calculate", command=calculate).grid(row=10, column=0, pady=10)
    ttk.Button(root, text="Load CSV & Fit β", command=load_csv).grid(row=10, column=1, pady=10)
    ttk.Button(root, text="Plot Cost Curve", command=plot_cost_tradeoff).grid(row=10, column=2, pady=10)
